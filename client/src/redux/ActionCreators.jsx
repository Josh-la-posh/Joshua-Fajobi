...
export const addToCart =
  (id, gallery, brand, prices, name, attributes, selectedAttribute, index) =>
  async (dispatch) => {
    // CART
    ...
    if (attributes.length === selectedAttribute.length) {
      ...
      const uniqueId = [];
      cart.forEach(
        (cartItem) => cartItem.id === id && uniqueId.push(cartItem.id)
      );
...
export const nextImg = (index, imageIndex) => async (dispatch) => {
  ...
  cart.forEach((cartItem, index) => {
    if (index === imdex) {
      cartItem.imageIndex =
        cartItem.gallery.length - 1 > cartItem.imageIndex
